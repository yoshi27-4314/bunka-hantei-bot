...
_fix_all_result = {"status": "not_started"}


def _fix_all_monday_data():
    """Monday.comの全データ修復（drive_url + Slack ID→スタッフ名変換）"""
    global _fix_all_result
    _fix_all_result = {"status": "running", "progress": "開始..."}
    try:
        from services.monday import update_monday_columns
        from config import STAFF_MAP

        service = get_drive_service()
        root_folder_id = os.environ.get("GOOGLE_DRIVE_FOLDER_ID", "")

        # Step 1: Driveの全フォルダ取得
        _fix_all_result["progress"] = "Driveフォルダを検索中..."
        all_drive_folders = {}
        if service and root_folder_id:
            ...
            for yymm in yymm_results.get("files", []):
        ...
        # Step 2: Monday.comの全アイテム取得（修復対象カラムを全部含む）
        _fix_all_result["progress"] = "Monday.comデータを取得中..."
        staff_columns = ["shuppinon_tantosha", "satsuei_tantosha", "toshosha", "konpo_tantosha"]
        all_col_ids = ["kanri_bango", "drive_url"] + staff_columns
        col_ids_str = '", "'.join(all_col_ids)
        query = f'''
        query ($board_id: ID!) {{
            boards(ids: [$board_id]) {{
        ...
        # Step 3: 修復実行
        _fix_all_result["progress"] = f"{len(items)}件のアイテムを修復中..."
        drive_fixed = []
        staff_fixed = []
        error_list = []
        ...
        for item in items:
            ...
            if not drive_url or not drive_url.startswith("https://"):
                folder_id = all_drive_folders.get(kanri)
                if folder_id:
                    updates["drive_url"] = f"https://drive.google.com/drive/folders/{folder_id}"

            # スタッフ名修復（Slack IDが入っていたら名前に変換）
            for col_id in staff_columns:
                val = cols.get(col_id, "")
                if val and val in STAFF_MAP:
                    updates[col_id] = STAFF_MAP[val]

            if updates:
                try:
                    update_monday_columns(kanri, updates)
                    if "drive_url" in updates:
                        drive_fixed.append(kanri)
                    staff_cols_fixed = [k for k in updates if k in staff_columns]
                    if staff_cols_fixed:
                        staff_fixed.append({"kanri_bango": kanri, "columns": staff_cols_fixed, "name": item["name"]})
                except Exception as e:
        ...
        _fix_all_result = {
            ...
            "drive_fixed": len(drive_fixed),
            "staff_name_fixed": len(staff_fixed),
            "staff_fixed_details": staff_fixed,
            "errors": len(error_list),
            "error_details": error_list,
        }
    except Exception as e:
        import traceback
        _fix_all_result = {"status": "error", "error": str(e), "traceback": traceback.format_exc()}


@app.route("/fix-all-20260325", methods=["GET"])
def fix_all_monday():
    """Monday.com全データ修復"""
    global _fix_all_result
    if _fix_all_result["status"] == "running":
        return jsonify({"message": "実行中です...", "progress": _fix_all_result.get("progress", "")})
    if _fix_all_result["status"] in ("done", "error"):
        if request.args.get("reset"):
            _fix_all_result = {"status": "not_started"}
            return jsonify({"message": "リセットしました。"})
        return jsonify(_fix_all_result)
    threading.Thread(target=_fix_all_monday_data, daemon=True).start()
    return jsonify({"message": "全修復を開始しました。60秒後にこのURLに再度アクセスしてください。"})
