...
if not GAS_URL:
    print("[警告] GAS_URL が未設定です。スプレッドシート連携が動きません")
...
　★スタッフは素人。「何をどうやって確認するか」を具体的に伝えること。
　★商品カテゴリに応じた確認項目を出すこと（全商品共通の汎用リストは禁止）。
　★予想販売価格によって確認の深さを変えること。

　【¥30,000未満の商品 → 簡易確認（1〜2分で終わる内容）】
　その商品で最も重要な1〜2点だけ確認させる。
　例（ラジカセ）：「コンセントを挿して、電源ランプがつくか確認してください」
　例（椅子）：「座ってみてガタつきがないか確認してください」
　例（食器）：「欠けやヒビがないか見てください」
...
　　確認したら → `B 電源つく` のように、ランク＋結果を返信」

　【¥30,000以上の商品 → 丁寧確認（3〜5分）】
　重要な3〜4点を確認させる。それでも手短に。
...
　　確認したら → `B 電源OK・リモコンあり` のように、ランク＋結果を返信」
...
★「現物確認ガイド」の書き方ルール（重要）★
スタッフは商品の素人。目の前に現物がある。
「この商品の何を見ればいいか」「どこが価値を上げるか・下げるか」を具体的に教えること。
AIの判定理由（市場分析・コスト計算）は書かない。スタッフが現物を手に取って確認すべき内容だけ書く。

現物確認ガイドに含めるべき内容：
・✅ 必ず確認：その商品で絶対にチェックすべき箇所（動作・通電・開閉・液漏れ等）
・⬆️ プラス要素：あれば価値が上がるもの（付属品・箱・説明書・純正パーツ・刻印等）
・⬇️ マイナス要素：あると価値が下がるもの（傷・サビ・欠品・変色・臭い・動作不良等）
・⚠️ 出品不可ライン：この状態なら通販ではなくロット/スクラップに変更すべき基準

商品カテゴリに応じて具体的に書くこと。例：
・家電 → 「通電するか」「リモコン・電源コードはあるか」「異音はないか」
・家具 → 「引き出しはスムーズに動くか」「ガタつきはないか」「天板の傷・輪染み」
・食器・陶器 → 「欠け・ヒビ・貫入」「裏印・窯印はあるか（あればプラス）」
・工具 → 「刃の摩耗」「動作確認」「バッテリーの充電可否」
・ブランド品 → 「真贋の確認ポイント（刻印・縫製・金具）」「製造番号の有無」
...
🔍 現物確認ガイド：
　✅ 必ず確認：[その商品で必ずチェックすべきこと2〜3点]
　⬆️ プラス：[あれば価値が上がるもの2〜3点]
　⬇️ マイナス：[あると価値が下がるもの2〜3点]
　⚠️ 出品不可：[この状態ならロット/スクラップに変更]
