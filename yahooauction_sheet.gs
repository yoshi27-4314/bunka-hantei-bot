...
// ====== 免責・注意事項文（共通） ======
const DISCLAIMER = `※ご入札前に必ずお読みください。

・基本的に商品は一点ものとなり、中古のアンティーク・ヴィンテージ品を数多く扱っております。
　動作についての保証は致しかねます。商品の状態は写真にてご確認ください。
・未使用やデッドストック品を含め、今後の動作の保証はできません。

・専門的な知識のない部品も数多くあり、正確な動作・状態・製作年代などを保証できません。
　機材の詳細な機能や仕様、塗装の変色や劣化、細かな傷につきましても正確な判断ができない場合がございます。
以上をご確認の上、全品ノークレームノーリターンでお願いいたします。
また、神経質な方は入札をご遠慮ください。

・お取引についてはご落札日より1週間以内にご連絡をお願いしております。
・同梱をご希望の場合、梱包サイズが変更となることがございますのでお問合せください。
　梱包後の3辺の合計が160cm/重量が25kgを超える場合は同梱できません。
　梱包後に配送料が変わることがございますのでご了承ください。
※同梱希望の方は、当方からの送料確定後のお支払いをお待ちください。
　先にお支払いされた場合、送料に差額が発生する場合がございます。

・入金確認後、おおよそ1週間以内に発送させていただいております。
　ご落札後1週間を超えてもご連絡がない場合は、お取引の意思がないものと判断し、
　落札者都合によるキャンセルとさせていただく場合がございます。
・お届けにつきまして、ご入金確認後1週間以内の発送とさせていただいております。
※大型商品や地域によっては配送業者の都合でお時間をいただく場合がございますのでご了承ください。

◆営業日：月・火・水・木・金 10:00〜16:00
　　　　　（土曜・日曜・祝日・その他特別休業日を除く）

上記営業日の関係上、即日発送や翌日発送などの早急なお届けには
対応できない場合がございますのでご了承ください。

また、祝日・年末年始等の特別期間はご対応が遅れる場合がございますのでご了承ください。
...
function _setupTemplateSheet(ss, existing) {
  ...
  const vintageTemplate = `{内部KW}

《商品説明》
{アイテム名}です。
{商品詳細}

・ヴィンテージ品でインテリアとしてもお楽しみいただけます。
・経年による汚れ、傷、擦れ、傷み、くすみ等がございます。写真にてご確認ください。
・専門的な鑑定は行っておりません。
・古いものですので完全な動作や品質を保証できません。
・メンテナンスや修理が必要な場合はお客様にてお願いいたします。
...
※サイズ及び重量については簡易計測のため正確ではございませんのでご了承ください。
...
【発送方法】
・発送は岐阜県からの宅配便にて発送になります。

{送料テーブル}

【取引詳細・その他】
・平日16:00以降及び土日祝はご回答・返答・発送等ができませんのでご連絡が遅くなります。
・ご入札をお考えの方でご不明な点がある方は、入札前にご質問よりご連絡ください。
・複数商品をお買い求めの方は落札後にまとめてご連絡ください。
・写真に写っているものが全てとなります。
・動作未保証のため返品はお受けできません。
・商品に関して神経質な方のご入札はお控えください。
...
function _setupSettingsSheet(ss, existing) {
  ...
  sh.getRange('A17').setFontWeight('bold').setBackground('#666666').setFontColor('white');
  sh.getRange('A24').setFontWeight('bold').setBackground('#666666').setFontColor('white');
  sh.getRange('A31').setFontWeight('bold').setBackground('#666666').setFontColor('white');
  // 出品担当マーク ヘッダー行
