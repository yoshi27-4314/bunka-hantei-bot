...
### 2026-03-20〜21：LOCATION_PATTERNのimportエラーでBot全停止（約24時間）

**発生期間：** 2026-03-20 08:12 〜 2026-03-21（修正デプロイまで）

**内容：**
Phase 1Aデプロイ後、Botが起動できず全チャンネルで応答なし。
Make.comヘルスチェックが502 Bad Gatewayを検知し通知。

**原因：**
`LOCATION_PATTERN = re.compile(...)` をモジュールレベル（グローバル定数）として追加したが、
`import re` がファイル先頭になかった（各関数内でローカルインポートしていたため見落とし）。
```
...
**対処：**
- 2026-03-21 `import re` をファイル先頭に追加（コミット 288f602）

**再発防止：**
- モジュールレベルで使用するライブラリは必ずファイル先頭でインポートすること
- デプロイ後にヘルスチェック（`/health`）の200応答を確認すること
...
| | 価格方針：値ごろ感スタート、入札1〜3件想定 | 浅野儀頼の方針決定 | | |
| 2026-03-20 | 価格方針を修正：古道具1,100円最低金額はあり得る | 1,100円スタートの全面否定ではなく、集客戦略としての安価スタートを不採用。商品相応なら1,100円で出品する | Claude Code | 4ca116f |
| 2026-03-20 | サイズ別最低出品金額テーブル（MIN_LISTING_PRICE）を追加 | コスト構造から逆算。赤字ライン×1.5倍。人件費は当初想定の2倍で補正 | Claude Code | 5ba4240 |
| | 60-80: ¥2,200 / 100-140: ¥4,500 / 160-200: ¥7,500 / 220+: ¥12,000 | 倉庫125㎡月額¥195K、給与¥420K、販管費¥85K、営業利益目標¥800K/月 | | |
| | これ以下の商品は個別出品しない→まとめ売りorスクラップへ | 保管コストの無駄を排除。早く売り切る方針 | | |
| 2026-03-21 | import re をファイル先頭に追加（起動時NameError修正） | LOCATION_PATTERNがモジュールレベルでre.compile()を使用していたが、reがトップレベル未インポートだったためBot起動不可。約24時間停止 | Claude Code | 288f602 |
| 2026-03-21 | 出品チャンネルに画像管理機能を追加 | Slack上で画像の確認・追加・削除・入替・撮り直しが完結。Drive共有不要 | Claude Code | fbf6e10 |
| | セッション開始時にDriveから商品画像を自動取得・表示（テプラ自動除外） | スタッフはSlackだけで作業完結 | | |
| | 画像コマンド: 画像削除/順番入替/撮り直し/追加撮影/一覧表示 | 写真投稿だけで追加撮影（コマンド不要） | | |
| | 出品チャンネルからの追加撮影は sp01_出品追加.jpg 形式でDrive保存 | 撮影チャンネルの画像（02_商品.jpg）と識別子で区別 | | |
| | Drive関連関数5つ新設（get_drive_folder_id/list_drive_images/delete_drive_file/upload_shuppinon_image/replace_drive_file） | | | |
| 2026-03-21 | ロケーション番号に倉庫コード（A=厚見/H=本荘/Y=柳津）を必須化 | 3倉庫体制。どの倉庫か不明だと出荷時に商品を探せない。浅野儀頼の指示 | Claude Code | e882dc2 |
| | 旧データ（倉庫コードなし）は「⚠️ 倉庫不明・要確認」と表示 | 自動補完はしない。スタッフが確認して修正 | | |
| 2026-03-21 | docs/出品保管_作業マニュアル.html V2 全面改訂 | 画像管理・倉庫コード・タイトル65文字・担当マーク・操作例3パターン。浅野儀頼の指示 | Claude Code | 048eb00 |
| | ヤフオク画面操作編は後日スタッフと一緒に作成予定 | スクリーンショットが必要なため | | |
