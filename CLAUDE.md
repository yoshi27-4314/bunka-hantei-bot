...
**コードを1行でも変更したら、必ず以下を記録すること。記録なしの変更は禁止。**
...
#### 記録しなければならない内容
1. **何を変えたか**（ファイル名・行番号・変更前→変更後）
2. **なぜ変えたか**（理由・誰の指示か）
3. **いつ変えたか**（日時）
4. **誰が変えたか**（Claude Code / Claude AI / 人間）
5. **Gitコミットハッシュ**（後から追跡できるように）

#### 特定ユーザー・チャンネルの除外・制限・特別扱いをする場合
- **浅野儀頼（CEO）の明示的な口頭または文書での承認が必須**
- 承認を得た日時・内容をこのファイルに必ず記録すること
- 承認なしに特定ユーザーを制限するコードを書くことは禁止

#### このルールができた背景（2026/03/18）
浅野儀頼のSlackメッセージをBotが無視する設定（ADMIN_USER_ID）が
理由・承認の記録なしにコードに書かれていた。
その結果、CEOが自分のシステムを使えない状態が発生した。
二度とこのような事態を起こさないためにこのルールを制定する。
