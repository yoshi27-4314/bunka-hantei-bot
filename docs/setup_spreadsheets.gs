...
function createChannelsSheet(ss) {
  ...
  const data = [
    ...
    ["eBayシングル", "通販", "発行（Eコード）", "8%以上", "海外需要のある希少品・ビンテージ。国内で安い商品が高額になることも", "手数料約13%。英語対応必要"],
    ["eBayまとめ", "通販", "発行（Eコード）", "20%前後", "単品では安い部品・同種類の複数品。まとめで付加価値", "組み合わせ判断力が必要"],
    ["ヤフオクビンテージ", "通販", "発行（Vコード）※旧形式", "8%以上", "昭和レトロ・アンティーク・コレクターズアイテム", "在庫期間が長くなりやすい"],
...
function createKanriNumberSheet(ss) {
  ...
  const data = [
    ...
    ["旧フォーマットの区分コード", "V=ヤフオクビンテージ / G=ヤフオク現行 / M=ヤフオクまとめ / E=eBay系"],
    ["旧フォーマットの扱い", "新規発行しない。既存の旧番号はシステムで参照・表示可能"],
    ["", ""],
    ["発行対象チャンネル", "eBayシングル・eBayまとめ・ヤフオクビンテージ・ヤフオク現行・ヤフオクまとめ"],
