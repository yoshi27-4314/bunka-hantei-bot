...
function _initStaffSheet(ss) {
  ...
  const headers = ['名前', 'SlackユーザーID', '時給（円）', '雇用区分', '査定係数', '備考'];
  const sample = [
    ['浅野儀頼', '', 0, '正社員', 1.0, 'SlackID確定後に入力'],
    ['林和人',   '', 0, '正社員', 1.0, 'SlackID確定後に入力'],
    ['横山優',   '', 0, 'パート',  0.9, 'SlackID確定後に入力'],
    ['平野光雄', '', 0, 'パート',  0.9, 'SlackID確定後に入力'],
    ['桃井',     '', 0, 'パート',  0.9, 'SlackID確定後に入力'],
    ['北瀬',     '', 0, 'パート',  0.9, 'SlackID確定後に入力'],
  ];
